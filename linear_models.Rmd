...
test.avg <- data_frame(pred.pls = pred.test.pls, pred.glmnet = pred.test.glmnet)
...
```{r}
d <- full_join(train.ica, train.mca, by = c("USER_ID", "Party"))
library(AppliedPredictiveModeling)
transparentTheme(trans = 0.4)
# ggplot(d, aes(x = ICA2, y = mca5, color = Party)) +
    # geom_point()
featurePlot(x = d[, 3:12], y = d$Party, plot = "pairs", auto.key = list(columns = 2))
```
